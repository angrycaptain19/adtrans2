...
class DataTrainingArguments:
    ...
    adapter_name: Optional[str] = field(
        default=None, metadata={"help": "adapter name for wandb"}
    )
...
def main():
    ...
    if (fusion_args.train_fusion + fusion_args.test_fusion) == 1 :
        ...
        adapter_names = [['default','xAttr','xEffect']]


        if fusion_args.train_fusion:
            if fusion_args.temperature:
                fusion_config = {
                    "key": True,
                    "query": True,
                    ...
                    "temperature": fusion_args.temperature,
                    "value": True,
        ...
        elif fusion_args.test_fusion:
            if fusion_args.fusion_path is None:
                raise RuntimeError("There is no fusion path for test.")

            fusion_path = fusion_args.fusion_path
            model.load_adapter_fusion(fusion_path)
            model.set_active_adapters(adapter_names) # is this code needed?
